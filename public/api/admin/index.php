...
if (logged_in() && isset($_GET['export'])) {
    ...
    $output = fopen('php://output', 'w'); fputcsv($output, ['予約番号','状態','出発日時','返却日時','料金体系','車両','氏名','メール','電話','受取場所','金額','追加サービス','作成日時']);
    foreach ($pdo->query('SELECT * FROM bookings ORDER BY created_at DESC') as $row) fputcsv($output, [$row['code'],$row['status'],$row['start_date'].' '.($row['start_time']??''),$row['end_date'].' '.($row['end_time']??''),$row['billing_mode']??'daily',$row['car_class'],$row['name'],$row['email'],$row['phone'],$row['pickup_location'],$row['total'],$row['additional_services'],$row['created_at']]);
    fclose($output); exit;
}
