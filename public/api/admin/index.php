...
if (logged_in() && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['mail_action'])) {
    ...
    elseif ($_POST['mail_action'] === 'test') {
        ...
        elseif (send_test_mail($testEmail, $config)) $message = 'テストメールをPHPのメールサーバーへ渡しました。受信箱と迷惑メールをご確認ください。';
        else $error = 'テストメールを送信できませんでした。下の送信履歴でエラー内容をご確認ください。';
    } elseif ($_POST['mail_action'] === 'resend') {
        $bookingId = (int)($_POST['mail_booking_id'] ?? 0);
        $stmt = $pdo->prepare('SELECT * FROM bookings WHERE id = :id');
        $stmt->execute([':id' => $bookingId]);
        $booking = $stmt->fetch();
        if (!$booking) $error = '再送する予約が見つかりません。';
        else {
            $newToken = bin2hex(random_bytes(18));
            $previousTokenHash = (string)$booking['access_token_hash'];
            $stmt = $pdo->prepare('UPDATE bookings SET access_token_hash = :token, updated_at = :now WHERE id = :id');
            $stmt->execute([':token' => hash('sha256', $newToken), ':now' => date('Y-m-d H:i:s'), ':id' => $bookingId]);
            $status = send_booking_mail($booking, $newToken, $config, 'created');
            if (!$status['customer']) {
                $stmt = $pdo->prepare('UPDATE bookings SET access_token_hash = :token WHERE id = :id');
                $stmt->execute([':token' => $previousTokenHash, ':id' => $bookingId]);
            }
            if ($status['customer'] && $status['admin']) $message = 'お客様と管理会社へ予約メールを再送しました。新しい管理キーへ更新されています。';
            elseif ($status['customer'] || $status['admin']) $error = '一方のメールのみ送信処理を受け付けました。下の送信履歴をご確認ください。';
            else $error = '予約メールを再送できませんでした。下の送信履歴をご確認ください。';
        }
    }
}
...
$bookings=[]; $blocks=[]; $vehicles=[]; $vehicleLabels=[]; $mailLogs=[]; $mailSettings=mail_config($config);
if (logged_in()) {
...
<section class="manager"><h2>メール送信確認</h2><p>予約メールの送信元と、PHPがメールサーバーへ受け渡した結果を確認できます。</p><div class="mail-summary"><div><small>差出人</small><strong><?=e($mailSettings['from'])?></strong></div><div><small>配送元（Return-Path）</small><strong><?=e($mailSettings['envelope'])?></strong></div><div><small>管理会社の通知先</small><strong><?=e($mailSettings['admin'])?></strong></div></div><form class="mail-test" method="post"><input type="hidden" name="csrf" value="<?=e($_SESSION['csrf'])?>"><input type="hidden" name="mail_action" value="test"><label>テスト送信先<input type="email" name="test_email" value="<?=e($mailSettings['admin'])?>" required></label><button>テストメールを送信</button></form><p class="mail-note">「受付」はPHPからメールサーバーへ渡せた状態です。実際の受信は、迷惑メール判定やSPF・DKIM・DMARC設定にも影響されます。</p><?php if($mailLogs):?><div class="table-wrap mail-log"><table><thead><tr><th>日時</th><th>予約番号</th><th>種別</th><th>送信先</th><th>方式</th><th>結果</th><th>エラー</th></tr></thead><tbody><?php foreach($mailLogs as $log):?><tr><td><?=e($log['created_at'])?></td><td><?=e($log['booking_code']?:'—')?></td><td><?=e($log['recipient_type'])?></td><td><?=e($log['recipient'])?></td><td><?=e($log['transport']?:'php-mail')?></td><td class="<?=$log['accepted']?'mail-ok':'mail-ng'?>"><?=$log['accepted']?'受付':'失敗'?></td><td><?=e($log['error_message']?:'—')?></td></tr><?php endforeach;?></tbody></table></div><?php else:?><p class="empty">まだメール送信履歴はありません。</p><?php endif;?></section>
